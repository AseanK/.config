  ...
  commonHosts.setEntries(['twitch.tv', 'player.twitch.tv', 'm.twitch.tv'], function pageCallback$i() {
    ...
    function hookWorkerFetch() {
      ...
      fetch = async function fetch(url, options) {
        ...
        if (url.includes('/api/channel/hls/') || url.includes('/api/v2/channel/hls/')) {
    ...
    function getTwitchPlayer() {
      ...
      const node = findReactNode(reactRootNode, n => n.setPlayerActive && n.props && n.props.mediaPlayerInstance);
      return (node === null || node === void 0 ? void 0 : (_node$props = node.props) === null || _node$props === void 0 ? void 0 : _node$props.mediaPlayerInstance) || null;
    }
    const button2kContent = '1440p602k';
    const qualityButtonsSelector = 'div[data-a-target="player-settings-menu"] div[role="menuitemradio"] div[data-a-target="player-settings-submenu-quality-option"] input[data-a-target="tw-radio"]';
    function get2kQualityButton() {
      const qualityOptionButtons = browser$1.querySelectorAll(qualityButtonsSelector);
      const button2k = Array.from(qualityOptionButtons).find(button => {
        let _button$nextSibling;
        const label = ((_button$nextSibling = button.nextSibling) === null || _button$nextSibling === void 0 ? void 0 : _button$nextSibling.textContent) || '';
        if (label.startsWith(button2kContent)) {
          return true;
        }
        return false;
      });
      return button2k || null;
    }
    function disable2kButton() {
      const button2k = get2kQualityButton();
      const blockingAdsElementIsVisible = browser$1.querySelector('.adblock-overlay[style="display: block;"]');
      if (button2k && blockingAdsElementIsVisible) {
        button2k.disabled = true;
        const divButton = button2k.closest('div[data-a-target="player-settings-menu"] > div:not([class])');
        if (divButton) {
          divButton.style.pointerEvents = 'none';
        }
      }
    }
    function enable2kButton() {
      const button2k = get2kQualityButton();
      const feature2kNotification = browser$1.querySelector('div[data-a-target="player-settings-menu"] .tw-in-feature-notification');
      if (button2k && !feature2kNotification) {
        button2k.disabled = false;
        const divButton = button2k.closest('div[data-a-target="player-settings-menu"] > div:not([class])');
        if (divButton) {
          divButton.style.pointerEvents = '';
        }
      }
    }
    const qualityMenuItemSelector = 'button[data-a-target="player-settings-menu-item-quality"]';
    window.addEventListener('click', event => {
      const {
        target
      } = event;
      if (!(target instanceof Element) || !target.closest(qualityMenuItemSelector)) {
        return;
      }
      requestAnimationFrame(disable2kButton);
    }, {
      passive: true
    });
    function is2kQualityStream(streamInfo) {
      let _streamInfo$codecs, _streamInfo$codecs2, _streamInfo$name;
      return (streamInfo === null || streamInfo === void 0 ? void 0 : (_streamInfo$codecs = streamInfo.codecs) === null || _streamInfo$codecs === void 0 ? void 0 : _streamInfo$codecs.startsWith('hev')) || (streamInfo === null || streamInfo === void 0 ? void 0 : (_streamInfo$codecs2 = streamInfo.codecs) === null || _streamInfo$codecs2 === void 0 ? void 0 : _streamInfo$codecs2.startsWith('hvc')) || (streamInfo === null || streamInfo === void 0 ? void 0 : (_streamInfo$name = streamInfo.name) === null || _streamInfo$name === void 0 ? void 0 : _streamInfo$name.startsWith('1440'));
    }
    function firstNot2kQualityStream(qualities) {
      return qualities.find(q => !is2kQualityStream(q));
    }
    let originalPlayerQuality = null;
    let wasAutoQualityModeEnabled = null;
    function forceChangeQuality(value) {
      try {
        const videoPlayer = getTwitchPlayer();
        if (!videoPlayer) {
          return;
        }
        ...
        if (value === 'original') {
          ...
          if (originalPlayerQuality) {
            targetQuality = qualities.find(q => q.group === originalPlayerQuality.group);
          }
          if (!targetQuality) {
            targetQuality = qualities.find(q => q.group === 'auto');
          }
          const currentQuality = videoPlayer.getQuality();
          const isCurrentlyInAutoQualityMode = videoPlayer.isAutoQualityMode();
          const shouldRestoreSavedQuality = targetQuality && targetQuality.name !== (currentQuality === null || currentQuality === void 0 ? void 0 : currentQuality.name) && !wasAutoQualityModeEnabled && !isCurrentlyInAutoQualityMode;
          const shouldRestoreAutoMode = wasAutoQualityModeEnabled && !isCurrentlyInAutoQualityMode;
          if (shouldRestoreSavedQuality) {
            videoPlayer.setQuality(targetQuality);
          } else if (shouldRestoreAutoMode) {
            videoPlayer.setAutoQualityMode(true);
          }
          originalPlayerQuality = null;
          wasAutoQualityModeEnabled = null;
        } else {
          const currentQuality = videoPlayer.getQuality();
          disable2kButton();
          const is2kQuality = is2kQualityStream(currentQuality);
          if (!is2kQuality) {
            return;
          }
          if (!originalPlayerQuality) {
            originalPlayerQuality = currentQuality;
          }
          const nonAutoQualities = qualities.filter(q => q.group !== 'auto');
          if (nonAutoQualities.length === 0) {
            return;
          }
          wasAutoQualityModeEnabled = videoPlayer.isAutoQualityMode();
          const firstNot2kQuality = firstNot2kQualityStream(nonAutoQualities);
          videoPlayer.setQuality(firstNot2kQuality);
        }
      } catch (e) {}
    }
      ...
      browser$1.window.Worker = class Worker extends oldWorker {
        constructor(twitchBlobUrl) {
          ...
          this.onmessage = e => {
            ...
            switch (key) {
              case 'ForceChangeQuality':
                forceChangeQuality(value);
                break;
