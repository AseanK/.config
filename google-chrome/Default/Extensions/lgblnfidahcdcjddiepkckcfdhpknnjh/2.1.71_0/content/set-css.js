...
async function getCssFromStorage() {
  ...
  if (pageData.enabled) {
    if (!whitelist[pageData.hostAddress]?.includes('*')) {
      ...
      const filterByWhitelist = rules => rules.filter(val => !whitelistRules.includes(val));
      generalRules = filterByWhitelist(generalRules);
      generalExtendedRules = filterByWhitelist(generalExtendedRules);
    }
  }
