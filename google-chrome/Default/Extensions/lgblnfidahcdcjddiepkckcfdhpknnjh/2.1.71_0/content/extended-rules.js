...
class ExtendedRules {
  ...
  hasExtendedPseudoClass(str) {
    return [':has(', ':-abp-has(', ':-abp-contains(', ':contains(', ':upward(', ':-content-before('].some(pseudo => str.includes(pseudo));
  }
  ...
  elementMatchesConditions(element, rule) {
    return this.checkNotConditions(element, rule) && this.checkHasConditions(element, rule) && this.checkTextCondition(element, rule) && this.checkBeforeCondition(element, rule);
  }
  ...
  checkBeforeCondition(element, rule) {
    if (!rule.before) return true;
    return this.checkBeforeContent(element, rule.before);
  }
  checkBeforeContent(element, beforeCondition) {
    const beforeContent = window.getComputedStyle(element, '::before').getPropertyValue('content');
    if (!beforeCondition.includes('|')) {
      return beforeContent.includes(beforeCondition);
    }
    return beforeCondition.split('|').some(part => {
      const trimmed = part.trim();
      if (trimmed.startsWith('/') || trimmed.endsWith('/')) {
        const content = trimmed.replace(/^\/|\/$/g, '');
        return beforeContent.includes(content);
      }
      return beforeContent.includes(trimmed);
    });
  }
